...
2. **ONNX Runtime must be loaded first**: The modern v0.17+ API requires loading `libonnxruntime` before `libvoicevox_core`, which requires an extra `FFI::cdef()` call.
...
**Latest Release**: v0.17.0 (August 2026)
**Language**: Rust  
...
The C API exposes **66 functions** across five logical groups[^2]:
...
### Initialization Sequence (Modern v0.17+ API)
...
The modern v0.17+ API requires **explicit** ONNX Runtime loading before anything else. Three strategies are available:[^11]
...
// e.g. "libvoicevox_onnxruntime.so.1.23.2"
...
|---------|------------|--------|
| C API has 66 functions | High | Direct header inspection |
...
[^1]: [VOICEVOX/voicevox_core Releases](https://github.com/VOICEVOX/voicevox_core/releases) — v0.17.0 release assets
